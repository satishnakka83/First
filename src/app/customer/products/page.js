...
function StorePageContent() {
    const searchParams = useSearchParams(); // Dynamically parse query hooks safely
    
    const initialGender = searchParams.get('gender') || 'Men';
    const initialItemName = searchParams.get('item_name') || '';
    ...
    // Sync state filters if the URL changes directly via hook properties
    useEffect(() => {
        setFilters({
        ...
        return (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/40 backdrop-blur-sm p-4 transition-all">
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-y-auto flex flex-col md:flex-row relative animate-in fade-in zoom-in-95 duration-200">
                    ...
                    <button
                        onClick={onClose}
                        className="absolute top-4 right-6 bg-red/80 backdrop-blur text-red-500 hover:text-gray-900 z-10 p-2 rounded-full w-8 h-8 shadow-sm transition-colors "
                    >
                        ✕
                    </button>
    ...
    return (
        <div className="w-full min-h-screen flex flex-col bg-white  text-gray-800">
            ...
            <div className="flex-grow max-w-[1600px] w-full mx-auto p-4 md:p-8">
...
export default function StorePage() {
    return (
        <Suspense fallback={<div className="w-full min-h-screen flex items-center justify-center bg-white text-gray-400">Loading collection...</div>}>
            <StorePageContent />
        </Suspense>
    );
}